<div class="form-floating mb-3">
  <input type="text"  value="{{ old('name', $kelas->name) }}"name="name" class="form-control text-black @error('name') is-invalid @enderror" id="floatingInput" placeholder="name@">
  ...
  <select name="kompetensikeahlian_id" class="form-select @error('kompetensikeahlian_id') is-invalid @enderror" id="floatingSelect" aria-label="Floating label select example">
    <option disabled selected>-- Pilih kompetensi keahlian --</option>
      @foreach ($kompetensikeahlian as $tampilkan)
        <option value=" {{ $tampilkan->id }} " {{ $tampilkan->id == $kelas->kompetensikeahlian_id ? 'selected' : '' }}> {{ $tampilkan->name }}</option>
      @endforeach
  </select>
