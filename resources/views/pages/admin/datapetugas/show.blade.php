        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <p class="m-0 d-inline font-weight-bold text-grey">Detail Petugas</p>
                                ...
                                <div class="text-center postion-relative">
                                    @if ($userphoto->count() > 0)
                                        <img src="/gallery/{{ $userphoto->first()->url }}"
                                            class="img-profile mb-3 rounded-circle" alt="{{ $petuga->name }}"
                                            style="width: 100px; height: auto; overflow: hidden;">
                                    @else
                                        <img src="/img/profil.png" class="img-profile mb-3 rounded-circle"
                                            alt="{{ $petuga->name }}" style="width: 100px;">
                                    @endif

                                    <form action="{{ route('photo-user.edit', $petuga->id) }}" method="GET"
                                        class="d-inline">
                                        @csrf
                                        <button class="position-absolute btn">
                                            ...
                                            <input type="hidden" name="redirect" id="redirect"
                                                value="{{ $redirect }}">
                                                <input type="hidden" name="name" id="name" value="{{ $petuga->name }}">
                                        </button>
                                    </form>
                                </div>
                            </tr>
